...
function traverseCell(cell, list, groupBy) {
    // console.log('> adsd', cell);
    // console.log('> listik', list);
    // console.log('==============================================');
    let acc = [cell];
...
module.exports = function (cells, beauty, groupBy) {
    beauty || (beauty = (arg) => arg);
    ...
    while (cells.length) {
        const cell = cells.shift();
        const group = traverseCell(cell, cells, groupBy);
        // acc.push(group.map(item => item.id || item.map(item2 => item2.id)));
        // groups.push(group);
